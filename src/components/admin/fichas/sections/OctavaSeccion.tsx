...
const OctavaSeccion = () => {
    ...
    useEffect(()=>{
      const fetchData = async () => {
          ...
          const elemento = infoDatosState.data.find((item: any) => item.IDDato === 54);
          const elemento1 = infoDatosState.data.find((item: any) => item.IDDato === 55);
          const elemento2 = infoDatosState.data.find((item: any) => item.IDDato === 56);
          ...
          const Datos = infoDatosState.data.map((obj: any) => {
            const primerC = infoDatosState.data.find((info:any)=>
              Number(info.IDDato) === Number(dataColums[0]?.id))
  ...
  return (
    <Box sx={{display:'flex',flexDirection:'column',gap:'1rem'}}>
      ...
      <Datatable columns={columns} rows={dataRows} getRowId={((row)=>row.frecuenciaCorte || Math.random())}/>
      <Box  sx={{ display: "flex", gap: "1rem", alignItems: "center" }}>
      <Box sx={{width:'50rem'}}>
          <Typography color={theme.palette.secondary.contrastText}>Mano de obra</Typography>
          <FormControlLabel sx={{color:theme.palette.secondary.contrastText}} control={<Checkbox  checked={valorTextF1 === "Familiar"} readOnly />} label="Familiar" />
          <FormControlLabel  checked={valorTextF1 === "Contratada"} control={<Checkbox />} label="Contratada" />
        </Box>
        <ReadOnlyTextField
          label="Precio"
          value={valorTextF2}
