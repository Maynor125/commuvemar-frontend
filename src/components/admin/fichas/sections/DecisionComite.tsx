...
const DecisionComite = () => {
  ...
  useEffect(() => {
    ...
    const elemento = infoDatosState.data.find(
      (item: any) => Number(item.IDDato) === 72
    );
    const elemento1 = infoDatosState.data.find(
      (item: any) => Number(item.IDDato) === 73
    );
    const elemento2 = infoDatosState.data.find(
      (item: any) => Number(item.IDDato) === 74
    );
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <Typography variant="h6" color={theme.palette.secondary.light}>
        Decisión del comité de aprobación y sanciones del SIC{" "}
      </Typography>
      <Box>
        {!hayDesicion && (
          <Box>
        ...
        <div style={{ position: "relative" }}>
          ...
          <div
            style={{
              ...
              left: 0,
              right: 0,
              borderBottom: `1px dashed ${theme.palette.secondary.contrastText}`,
            }}
          />
        </div>
        <div style={{ position: "relative" }}>
