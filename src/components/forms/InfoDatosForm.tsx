...
const InfoDatosForm:React.FC<GeneralActionProps> =({
    ...
    const resolver: Resolver<infoDatoInterface, FieldErrors<infoDatoInterface>> = async (
        data
      ) => {
        try {
      ...
      const createInfoDatos = async(informacion:string,descripcion:string,IDDato:number,
        IDFicha:number)=>{
           try {
            const response = await createInfoDato(informacion,
                descripcion,IDDato,IDFicha
            )
            if(!response.error){
                onClick();
            }
